...
public class MinCostFromSourceToDestination {
    ...
    private static int findMinCost(int[][] cost) {
        ...
        while (!minHip.isEmpty()) {
            ...
            if (currentCity == len - 1) {
                return currentDistance;
            }

            // Explore neighbors
            for (int nextCity = 0; nextCity < len; nextCity++) {

                if (!visited[nextCity] && cost[currentCity][nextCity] > 0) {

                    int newDist = currentDistance + cost[currentCity][nextCity];
