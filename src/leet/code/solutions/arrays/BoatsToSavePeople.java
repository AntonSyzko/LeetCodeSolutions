    ...
    private static int numRescueBoats(int[] people, int limit) {
        ...
        while (lightestPersonIndex <= heaviestPersonIndex) {
          ...
          if (people[lightestPersonIndex] + people[heaviestPersonIndex] <= limit){
              ...
              heaviestPersonIndex--; // if combined weights exceeded limit -> we can take only one person - heaviest one

            }
            boats++;//anyways we have taken ONE boat - two person or one ( the heaviest one)
