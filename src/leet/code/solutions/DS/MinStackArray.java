...
public class MinStackArray {
    ...
    public void push(int x) {
        ...
        if(array.length >= this.capacity){
            array = Arrays.copyOf(array, array.length * 2);//resize
        }
    ...
    public void pop() {
        if(index > -1){
            if (index == array.length/2 && array.length > this.capacity) {//shrink
                array = Arrays.copyOf(array, array.length / 2);
            }

            if(index > 0 && array[index] == currentMin){
                currentMin = array[index -1];//if popped one was smallest - set SMALLEST to previous
            }
