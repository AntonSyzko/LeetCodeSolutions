...
//        System.out.println(res);
    ...
    private static boolean wordBreakDP(String s, List<String> wordDict) {
        ...
        for(int inWordIndex = 1; inWordIndex <= s.length(); inWordIndex++){//start from 1
            for(int fromStart = 0; fromStart < inWordIndex; fromStart++){//start from 0

                if(DP[fromStart] //if curr DP[fromStart] = true
                        &&
                        dict.contains(s.substring(fromStart,inWordIndex))){

                    DP[inWordIndex] = true;

                    break;//one true entry is enough
                }
            }
        }
        return DP[s.length()];
